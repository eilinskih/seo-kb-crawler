...
Status: In progress
Summary:
- Added explicit bounded retry/backlog controls to the Operator Console.
- Retry controls use the existing URL Frontier and Content Processing dispatch
  API endpoints instead of introducing direct queue or database operations.
- UI copy distinguishes current eligible retry/backlog dispatch from future
...
- git diff --check
Next step:
- Validate and merge the retry controls slice, then perform #182 close-out
  synchronization.
...
Status: In progress
Summary:
- Added read-only Operator Console failure detail pages for Content Processing
  and Chunking.
- Jobs/readiness rows now link to failure detail pages where owning read models
  expose recent failure rows.
- Failure detail pages show total runs, retryable/terminal counts, status
  counts and recent failure details without adding retry semantics.
Changed files:
- docs/progress.md
- docs/operator-console-model.md
...
Validation:
- npm test -- --runTestsByPath apps/operator-console/src/operator-console.service.spec.ts apps/operator-console/src/operator-console.controller.spec.ts
- npm run build
- npm test
- git diff --check
Next step:
- Validate and merge the failure detail slice, then continue #182 with
...
Status: In progress
Summary:
- Added read-only Operator Console provider detail pages.
- Provider status rows now link to provider-specific detail pages.
- Provider detail shows status, tier, capabilities and warnings through the
  existing External SEO Data Providers service boundary.
Changed files:
- docs/progress.md
- docs/operator-console-model.md
- apps/operator-console/src/operator-console.controller.ts
...
- git diff --check
Next step:
- Validate and merge the provider detail slice, then continue #182 with richer
  failure detail and retry controls.

Date: 2026-08-06
Issue: #182
Status: In progress
Summary:
- Added read-only Operator Console topic detail pages.
- Topic detail loads through the existing Topic API and URL Frontier status API.
- Topic table rows now link to their detail pages.
- Detail pages show current topic configuration and topic-scoped frontier
  status while preserving existing bounded configuration/lifecycle forms.
Changed files:
- docs/progress.md
- docs/operator-console-model.md
...
- git diff --check
Next step:
- Validate and merge the topic detail slice, then continue #182 with richer
  failure/provider detail and retry controls.
...
Status: In progress
Summary:
- Started Operator Console production hardening and review workflows.
- Added token-based internal access control for the Operator Console HTTP
  surface.
- `OPERATOR_CONSOLE_ACCESS_TOKEN` enables authenticated access through
  `Authorization: Bearer` or `x-operator-console-token`.
- Production without a configured token fails closed; local development without
  a token remains available with an explicit warning.
- Updated Operator Console model to move authenticated access out of deferred
  scope.
Changed files:
- docs/progress.md
- docs/operator-console-model.md
- apps/operator-console/src/operator-console-access-control.service.ts
- apps/operator-console/src/operator-console-access-control.service.spec.ts
- apps/operator-console/src/operator-console-auth.guard.ts
- apps/operator-console/src/operator-console.controller.ts
- apps/operator-console/src/operator-console.module.ts
- apps/operator-console/src/operator-console.service.ts
- apps/operator-console/src/operator-console.service.spec.ts
Validation:
- npm test -- --runTestsByPath apps/operator-console/src/operator-console-access-control.service.spec.ts apps/operator-console/src/operator-console.service.spec.ts
- npm run build
- npm test
- git diff --check
Next step:
- Validate and merge the access-control slice, then continue #182 with review
  queue/read-model design and implementation.
...
Status: Review needed
Summary:
- Added durable Knex repository implementation for External Entity enrichment
  packs using the existing Issue #17 persistence tables.
- Repository stores provider source snapshots, enrichment attempts and
  external ID observations without approving aliases or ontology types.
- Latest pack retrieval reconstructs provider-neutral records from persisted
  attempt and external ID rows.
- Synchronized the External Entity model to remove the stale deferred Knex
  repository item.
Changed files:
- docs/progress.md
- docs/external-entity-enrichment-model.md
...
- packages/external-entity-enrichment/src/persistence/knex-external-entity-enrichment.repository.spec.ts
Validation:
- npm test -- --runTestsByPath packages/external-entity-enrichment/src/persistence/knex-external-entity-enrichment.repository.spec.ts packages/external-entity-enrichment/src/external-entity-enrichment.service.spec.ts
- npm run build
- npm test
- git diff --check
Next step:
- Validate and merge the durable persistence slice, then continue Issue #181
  with provider execution policy configuration close-out.
...
Status: Review needed
Summary:
- Added optional Google Knowledge Graph Search API execution behind configured
  API credentials.
- Provider uses fixture-backed normalizer and remains misconfigured/skipped
  when credentials are absent.
- Provider transport failures are allowed to throw so the enrichment service can
  record degraded provider warnings while local Schema.org fallback continues.
- Added environment-backed provider factory and module wiring.
Changed files:
- docs/progress.md
- docs/external-entity-enrichment-model.md
- packages/external-entity-enrichment/src/external-entity-enrichment.module.ts
- packages/external-entity-enrichment/src/external-entity-provider.factory.ts
- packages/external-entity-enrichment/src/external-entity-provider.factory.spec.ts
- packages/external-entity-enrichment/src/index.ts
- packages/external-entity-enrichment/src/providers/google-knowledge-graph.provider.ts
- packages/external-entity-enrichment/src/providers/google-knowledge-graph.provider.spec.ts
Validation:
- npm test -- --runTestsByPath packages/external-entity-enrichment/src/providers/google-knowledge-graph.provider.spec.ts packages/external-entity-enrichment/src/external-entity-provider.factory.spec.ts packages/external-entity-enrichment/src/external-entity-enrichment.service.spec.ts
- npm run build
- npm test
- git diff --check
Next step:
- Validate and merge the Google Knowledge Graph provider slice, then continue
  Issue #181 with Wikidata API/SPARQL execution.

Date: 2026-08-06
Issue: #181
Status: Review needed
Summary:
- Added the first External Entity provider execution validation slice.
- Followed the issue comment validation plan: probed providers, documented
  observed response behavior and added sanitized offline fixtures before
  concrete live adapters.
- Live probes confirmed Google Knowledge Graph and Wikidata have different
  ambiguity behavior, and long-tail phrases can produce weak or no public
  entity candidates.
- Added reusable Google Knowledge Graph and Wikidata normalizers covered by
  offline fixture contract tests.
Changed files:
- docs/progress.md
- docs/external-entity-enrichment-model.md
- packages/external-entity-enrichment/src/index.ts
- packages/external-entity-enrichment/src/providers/__fixtures__/**
- packages/external-entity-enrichment/src/providers/google-knowledge-graph.normalizer.ts
- packages/external-entity-enrichment/src/providers/provider-fixture-normalizers.spec.ts
- packages/external-entity-enrichment/src/providers/wikidata.normalizer.ts
Validation:
- npm test -- --runTestsByPath packages/external-entity-enrichment/src/providers/provider-fixture-normalizers.spec.ts packages/external-entity-enrichment/src/external-entity-enrichment.service.spec.ts
- npm run build
- npm test
- git diff --check
Next step:
- Validate and merge the fixture/normalizer slice, then continue Issue #181
  with optional Google Knowledge Graph live execution.
...
Status: Review needed
Summary:
- Architecture Steward close-out review found that Demand Engine mapping was
  collapsing External SEO fallback and owned-data observations into paid
  provider semantics.
- Fixed External SEO Demand adapter mapping so fallback signals remain
  `fallback`, Google Search Console owned performance data becomes
  `owned_data`, and owned metrics use `owned_data_backed`.
- Added regression coverage for fallback-only and Google Search Console
  owned-data demand paths.
- Synchronized project map with #179 persistence, GSC adapter and refresh
  service implementation state.
Changed files:
- docs/progress.md
- docs/project-map.md
- packages/demand-engine/src/external-seo-demand.provider.ts
...
- git diff --check
Next step:
- Validate, merge close-out stabilization and close Issue #179 if checks pass.
...
Status: Review needed
Summary:
- Architecture Steward close-out review found one blocker: pack references and
  Research Scheduling plans must not count as structured fallback content.
- Tightened SEO Agent Gateway retrieval-only safeguard input so provider calls
  require actual structured generation context.
- Added regression coverage ensuring source pack references and research plans
  alone do not call providers.
- Synchronized project map and SEO Agent Gateway storage wording with #184
  implementation state.
Changed files:
- docs/progress.md
- docs/project-map.md
- docs/seo-agent-gateway-model.md
- packages/seo-agent-gateway/src/seo-agent-gateway.service.ts
- packages/seo-agent-gateway/src/seo-agent-generation-runtime.service.spec.ts
Validation:
- npm test -- --runTestsByPath packages/seo-agent-gateway/src/seo-agent-gateway.service.spec.ts packages/seo-agent-gateway/src/seo-agent-generation-runtime.service.spec.ts packages/seo-agent-gateway/src/persistence/seo-agent-gateway.repository.spec.ts
- npm run build
- npm test
- git diff --check
Next step:
- Validate, merge close-out stabilization and close Issue #184 if checks pass.
...
Status: Review needed
Summary:
- Added Research dispatch execution boundary for persisted dispatch plans.
- Dispatch commands execute through an explicit adapter port and produce
  fail-safe receipts.
- URL Frontier dispatch exceptions are classified as enqueue failures for
  Research Operations health without making queue depth durable state.
- Synchronized the Research Scheduling model with the execution boundary.
Changed files:
- docs/progress.md
- docs/research-engine-scheduling-model.md
- packages/research-scheduling/**
...
- git diff --check
Next step:
- Run targeted and full validation, review and merge the dispatch execution
  boundary, then run #180 close-out review.
...
Status: Review needed
Summary:
- Added deterministic scheduler tick planning from scheduler control state and
  Topic snapshots.
- Ensured disabled or paused scheduler state produces skipped ticks with no
  background allocations.
- Ensured enabled scheduler state uses the existing fair background budget
  allocator without leasing URLs, enqueueing jobs or calling providers.
- Synchronized the Research Scheduling model with the tick-planning boundary.
Changed files:
- docs/progress.md
- docs/research-engine-scheduling-model.md
- packages/research-scheduling/**
...
- git diff --check
Next step:
- Run targeted and full validation, review and merge the scheduler tick planner
  boundary.
...
Status: Review needed
Summary:
- Added fail-safe scheduler control state contracts for `enabled`, `paused`
  and `disabled` execution.
- Added operator-safe control service semantics: missing state defaults to
  disabled, and pausing/disabling requires an auditable reason.
- Extended the in-memory Research Scheduling repository with scheduler control
  state persistence for tests and future durable implementation.
- Synchronized the Research Scheduling model with scheduler control ownership.
Changed files:
- docs/progress.md
- docs/research-engine-scheduling-model.md
- packages/research-scheduling/**
...
- git diff --check
Next step:
- Run targeted and full validation, review and merge the scheduler control
  state boundary.
...
Status: Review needed
Summary:
- Added URL Frontier operations telemetry for expired leases, eligible backlog
  and oldest eligible backlog age from durable Frontier state.
- Added a Research Operations adapter that combines URL Frontier telemetry with
  explicit scheduler/dispatch enqueue failure counts.
- Documented that queue enqueue failures remain scheduler/dispatch
  observability signals and are not derived from URL Frontier state.
Changed files:
- docs/progress.md
- docs/research-engine-scheduling-model.md
- docs/url-frontier-model.md
...
- packages/url-frontier/**
Validation:
- npm test -- --runTestsByPath packages/url-frontier/src/persistence/knex-url-frontier.repository.spec.ts packages/research-scheduling/src/research-scheduling.service.spec.ts
- npm run build
- npm test
- git diff --check
Next step:
- Run targeted and full validation, review and merge the URL Frontier telemetry
  boundary.
...
Status: Review needed
Summary:
- Added configurable retry policy extraction from URL Frontier crawl policy
  snapshots.
- Added bounded deterministic retry jitter while preserving maximum backoff.
- Synchronized URL Frontier documentation with current retry/backoff behavior.
- Kept operator-facing retry policy editing, scheduler automation and broader
  adaptive recrawl loops out of this slice.
Changed files:
- packages/url-frontier/**
- docs/url-frontier-model.md
- docs/progress.md
...
- git diff --check
Next step:
- Review and merge retry/recrawl policy hardening, then run Issue #3 close-out
  review.
...
Status: Review needed
Summary:
- Added a bounded URL Frontier candidate reevaluation service.
- Converted pending discovery observations into frontier entries using Topic
  configuration snapshots, crawl policy checks and pre-crawl relevance
  evidence.
- Linked evaluated observations to created or updated frontier entries.
- Kept automatic scheduling daemons, canonical consolidation, adaptive
  recrawl, retry policy hardening and Discovery Sources runtime integration out
  of this slice.
Changed files:
- packages/url-frontier/**
- docs/progress.md
Validation:
- npm test -- --runTestsByPath packages/url-frontier/src/application/url-frontier-reevaluation.service.spec.ts packages/url-frontier/src/persistence/knex-url-frontier.repository.spec.ts
- npm run build:api
- npm run build
- npm test
- git diff --check
Next step:
- Review and merge candidate reevaluation, then continue with canonical
  relation persistence and consolidation.

Date: 2026-07-23
Issue: #3
Status: Review needed
Summary:
- Added append-only URL discovery observation storage owned by URL Frontier.
- Added a Frontier-owned discovery observation DTO and repository append
  boundary compatible with Discovery Sources candidate observations.
- Persisted normalized URL identity and source/provenance metadata without
  creating new crawl jobs or changing crawler behavior.
- Kept candidate reevaluation, canonical consolidation, adaptive scheduling and
  retry policy hardening out of this slice.
Changed files:
- packages/db/src/db.service.ts
- packages/db/src/migrations/013-url-frontier-observations.ts
- packages/url-frontier/**
- docs/progress.md
Validation:
- npm test -- --runTestsByPath packages/url-frontier/src/persistence/knex-url-frontier.repository.spec.ts packages/url-frontier/src/application/url-frontier-dispatch.service.spec.ts
- npm run build:api
- npm run build
- npm test
- git diff --check
Next step:
- Review and merge observation ingestion, then continue with candidate
  reevaluation and richer candidate status transitions.
...
Status: Review needed
Summary:
- Added provider/fallback status visibility to the Operator Console.
- Integrated the External SEO Data Providers service boundary into the console
  view model.
- Rendered provider-neutral status, tier, capabilities and fallback/degraded
  warnings without calling paid-provider SDKs or requiring credentials.
- Kept provider credentials management, concrete paid provider integrations,
  direct provider schema reads and richer provider detail screens out of scope.
Changed files:
- apps/operator-console/**
- docs/operator-console-model.md
- docs/progress.md
...
- git diff --check
Next step:
- Review and merge the provider status slice before continuing remaining Issue
